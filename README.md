...
| [`docs_downloader.py`](docs_downloader.py) | Crawls a docs site / wiki / blog and saves each page as **clean plain text** in a local directory mirror. Strips scripts/styles/nav chrome. Sanitizes URLs to safe Linux filenames. Bypasses Burp on purpose. Two modes: `--filter critical` uses **per-site URL allowlists** for known sources (PortSwigger Academy lives at `/web-security/...`, OWASP at `/cheatsheets/`, etc. — not keyword guessing) and falls back to **code-block density** + URL/body heuristics for unknown sites; `--filter all` mirrors everything. `--list-presets` shows known sites; `--dry-run` previews. | Pre-exam: build a greppable payload vault you can `grep -ri 'time-based' vault/` during the timed exam |
