...
| [`docs_downloader.py`](docs_downloader.py) | Crawls a docs site / wiki / blog and saves each page as **clean plain text** in a local directory mirror. Strips scripts/styles/nav chrome. Sanitizes URLs to safe Linux filenames (no `/` `?` `*` etc. that break mkdir). Bypasses Burp on purpose. Two modes: `--filter critical` saves only vuln/exam-relevant pages (URL + body keyword match); `--filter all` mirrors everything. `--dry-run` previews what would be saved. | Pre-exam: build a greppable payload vault you can `grep -ri 'time-based' vault/` during the timed exam |
